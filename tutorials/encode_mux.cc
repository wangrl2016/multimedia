    ...
    switch ((*codec)->type) {
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            if (c->codec_id == AV_CODEC_ID_MPEG1VIDEO) {
                // Needed to avoid using macro-blocks in which some coefficient overflows.
...
static void OpenAudio(const AVCodec* codec, OutputStream* ost, AVDictionary* opt_arg) {
    AVCodecContext* c;
    int nb_samples;
    int ret;
    ...
    ost->t = 0;
    ost->incr = float(2 * M_PI * 110.0 / c->sample_rate);
    // increment frequency by 110 Hz per second
    ost->incr2 = float(2 * M_PI * 110.0 / c->sample_rate / c->sample_rate);
    ...
    av_opt_set_channel_layout(ost->swr_ctx,
                              "in_channel_layout", int64_t(c->channel_layout), 0);
    av_opt_set_int(ost->swr_ctx,
                   "in_sample_rate", c->sample_rate, 0);
    av_opt_set_sample_fmt(ost->swr_ctx,
                          "in_sample_fmt", AV_SAMPLE_FMT_S16, 0);
    av_opt_set_channel_layout(ost->swr_ctx,
                              "out_channel_layout", int64_t(c->channel_layout), 0);
    av_opt_set_int(ost->swr_ctx,
                   "out_sample_rate", c->sample_rate, 0);
    av_opt_set_sample_fmt(ost->swr_ctx,
                          "out_sample_fmt", c->sample_fmt, 0);
...
// Prepare a 16 bits dummy audio frame of 'frame_size' samples and
...
static AVFrame* GetAudioFrame(OutputStream* ost) {
    ...
    for (j = 0; j < frame->nb_samples; j++) {
        v = int16_t(sin(ost->t) * 10000);
        for (i = 0; i < av_get_channel_layout_nb_channels(ost->enc->channel_layout); i++)
            *q++ = v;
        ost->t += ost->incr;
        ost->incr += ost->incr2;
    }
...
static int WriteAudioFrame(AVFormatContext* oc, OutputStream* ost) {
    AVCodecContext* c;
    AVFrame* frame;
    int ret;
...
static void OpenVideo(const AVCodec* codec, OutputStream* ost, AVDictionary* opt_arg) {
    int ret;
...
static void FillYUVImage(AVFrame* pict, int frame_index,
...
static AVFrame* GetVideoFrame(OutputStream* ost) {
    ...
    if (c->pix_fmt != AV_PIX_FMT_YUV420P) {
        ...
        FillYUVImage(ost->tmp_frame, int(ost->next_pts), c->width, c->height);
        sws_scale(ost->sws_ctx, (const uint8_t* const*) ost->tmp_frame->data,
                  ost->tmp_frame->linesize, 0, c->height, ost->frame->data,
                  ost->frame->linesize);
    } else {
        FillYUVImage(ost->frame, int(ost->next_pts), c->width, c->height);
    }
...
// return 1 when encoding is finished, 0 otherwise.
static int WriteVideoFrame(AVFormatContext* oc, OutputStream* ost) {
...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        printf("usage: %s output_file\n"
               "API example program to output a media file with avformat.\n"
               "This program generates a synthetic audio and video stream, encodes and\n"
               "mux them into a file named output_file.\n"
