...
int main([[maybe_unused]] int argc, [[maybe_unused]] char* argv[]) {
    ...
    // - AddFontFromFileTTF() will return the ImFont* so you can store it if you need
    // to select the font among multiple.
    ...
    // - The fonts will be rasterizer at a given size (w/ oversampling) and stored into a
    // texture when calling ImFontAtlas::Build()/GetTexDataAsXXXX(),
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // We use Begin/End pair to create a named window.
        {
            static float f = 0.0f;
