...
static int OpenInputFile(const char* filename,
    ...
    // Initialize the stream parameters with de-muxer information.
    ...
    if ((error = avcodec_open2(codec_ctx, input_codec, nullptr)) < 0) {
...
static int OpenOutputFile(const char* filename,
    ...
    codec_ctx->channel_layout = av_get_default_channel_layout(OUTPUT_CHANNELS);
    codec_ctx->sample_rate = input_codec_context->sample_rate;
    ...
    if ((error = avcodec_open2(codec_ctx, output_codec, nullptr)) < 0) {
        fprintf(stderr, "Could not open output codec (error '%s')\n",
                av_err2str(error));
        goto cleanup;
    }

    error = avcodec_parameters_from_context(stream->codecpar, codec_ctx);
    if (error < 0) {
 ...
 * @return Error code (0 if successful)
 */
static int InitResampler(AVCodecContext* input_codec_context,
                         AVCodecContext* output_codec_context,
    ...
    *resample_context = swr_alloc_set_opts(nullptr,
                                           int64_t(output_codec_context->channel_layout),
                                           output_codec_context->sample_fmt,
                                           output_codec_context->sample_rate,
                                           int64_t(input_codec_context->channel_layout),
 ...
 * @return Error code (0 if successful)
 */
static int InitFifo(AVAudioFifo** fifo, AVCodecContext* output_codec_context) {
 ...
 * @return Error code (0 if successful)
 */
static int WriteOutputFileHeader(AVFormatContext* output_format_context) {
    int error;
 ...
 * @return Error code (0 if successful)
 */
static int ConvertSamples(const uint8_t** input_data,
...
static int ReadDecodeConvertAndStore(AVAudioFifo* fifo,
    ...
    if (data_present) {
        ...
        if (ConvertSamples((const uint8_t**) input_frame->extended_data,
                           converted_input_samples,
                           input_frame->nb_samples, resampler_context))
            goto cleanup;

        // Add the converted input samples to the FIFO buffer for later processing.
        if (AddSamplesToFifo(fifo, converted_input_samples,
                             input_frame->nb_samples))
            goto cleanup;
    }
    ret = 0;
...
static int EncodeAudioFrame(AVFrame* frame,
    ...
    error = InitPacket(&output_packet);
    if (error < 0)
        return error;
    ...
    *data_present = 0;
    ...
    // 1. If the last frame has been encoded, stop encoding.
    // 2. If the encoder asks for more data to be able to provide an
...
static int LoadEncodeAndWrite(AVAudioFifo* fifo,
    ...
    if (av_audio_fifo_read(fifo, (void**) output_frame->data,
                           frame_size) < frame_size) {
 ...
 * @return Error code (0 if successful)
 */
static int WriteOutputFileTrailer(AVFormatContext* output_format_context) {
    int error;
...
int main(int argc, char** argv) {
    ...
    if (OpenInputFile(argv[1], &input_format_context,
                      &input_codec_context))
        goto cleanup;
    // Open the output file for writing.
    if (OpenOutputFile(argv[2], input_codec_context,
                       &output_format_context, &output_codec_context))
        goto cleanup;
    // Initialize the resampler to be able to convert audio sample formats.
    if (InitResampler(input_codec_context, output_codec_context,
                      &resample_context))
        goto cleanup;
    // Initialize the FIFO buffer to store audio samples to be encoded.
    if (InitFifo(&fifo, output_codec_context))
        goto cleanup;
    // Write the header of the output file container.
    if (WriteOutputFileHeader(output_format_context))
        goto cleanup;
    ...
    while (true) {
        ...
        while (av_audio_fifo_size(fifo) < output_frame_size) {
            ...
            if (ReadDecodeConvertAndStore(fifo, input_format_context,
                                          input_codec_context,
        ...
        while (av_audio_fifo_size(fifo) >= output_frame_size ||
            ...
            if (LoadEncodeAndWrite(fifo, output_format_context,
                                   output_codec_context))
                goto cleanup;
    ...
    // Write the trailer of the output file container.
    if (WriteOutputFileTrailer(output_format_context))
        goto cleanup;
    ret = 0;
